...
namespace ZdravoCorp.Core.Scheduling.Schedule.Repository
{
    ...
    public class Schedule
    {
        ...
        public void CancelAppointmentsByRoomId(string roomId)
        {
            ...
            {
                if (appointment.RoomId == roomId && appointment.Status != AppointmentStatus.FINISHED)
                {
                    appointment.Status = AppointmentStatus.CANCELED;
                }
            }
        ...
        {
            return (from appointment in Appointments where appointment.RoomId == roomId && appointment.Status != AppointmentStatus.FINISHED select appointment.TimeSlot).ToList();
        }
        ...
        public string GenerateAppointmentId()
        {
            while (true)
            {
                string appointmentId = Generator.GenerateRandomId();
        ...
        public bool IsAvailable(string patientJmbg, TimeSlot duration)
        {
            List<Appointment>? patientAppointments = GetAppointmentsByPatientJmbg(patientJmbg);
            foreach (Appointment appointment in patientAppointments)
            {
                if (appointment.TimeSlot.OverlapsWith(duration) && (appointment.Status != AppointmentStatus.CANCELED))
                {
        ...
        public bool IsAppointmentInRange(string doctorJmbg,DateTime start, DateTime end)
        {
            foreach (var appointment in Appointments)
            {
                if (appointment.JmbgDoctor == doctorJmbg && appointment.Status == AppointmentStatus.SCHEDULED)
                {
                    if (appointment.TimeSlot.Start >= start && appointment.TimeSlot.Start <= end.AddDays(1))
                    {
                        return true;
                    }
                }
        ...
        {
            return (IsAvailable(doctor, timeSlot) && IsAvailable(patientJmbg, timeSlot));
        }

        public (TimeSlot, string) IsDoctorAvailableForAppointmentIn10Days(Doctor doc, DateTime start,string patientJmbg)
        {
            RoomService roomService = new RoomService();
            DateTime temp = start;

            while (temp < start.AddHours(10))
            {
                TimeSlot timeSlot = new TimeSlot(temp, temp.AddMinutes(15));
                Room room = roomService.FindFirstAvailableRoom(RoomType.ExaminationRoom, Appointments, timeSlot);

                if (AreDoctorAndPatientAvailable(doc, timeSlot, patientJmbg)) return (timeSlot, room.Id);
                temp = temp.AddMinutes(1);
            }
            return (null, null);
        }
        ...
        {
            (TimeSlot ts, string room) = IsDoctorAvailableForAppointmentIn10Days(doctor,start,patientJmbg);

            if (ts != null)
            {
                CreateAppointment(doctor.Jmbg, patientJmbg, ts, AppointmentType.EXAMINATION, room);
                return true;
            }
            return false;
        }
